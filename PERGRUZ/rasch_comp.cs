...
namespace PERGRUZ
{
    public partial class rasch_comp : UserControl
    {
        ...
        public karta karta_ = new karta();//инициализация карты
        public blank_Rascheta_vreda Rasch_Vred_ = new blank_Rascheta_vreda();//инициализация формы вывода расчета на бланкх exel
        ...
        public rasch_comp()
        {
         ...
         //   this.MinimumSize = new System.Drawing.Size(798, 794);
            this.Name = "rasch_comp";
         //   this.Size = new System.Drawing.Size(1023, 856);
            this.TabIndex = 0;
            karta_.Show();
            karta_.Hide();
        ...
        {
          //  karta_.Show();
            ShowWindow(karta_.Handle, 3); //выносим окно на передний план
        }
        ...
        private void RaschetNosi_Click()
        {
            ...
            for (int i = 2; i < kol_osi - 1; i = i + 2)
            {
                try
                {
                    if (worksheet[2, i] != null && worksheet[3, i - 1] != null && worksheet[3, i + 1] != null)
                    {
                        var test = worksheet[2, i];
                    ...
                    else
                    {
                        ...
                        MessageBox.Show( "ЗАПОЛНИТЕ ВСЕ ПАРАМЕТРЫ ПО ОСЯМ " + (i - 1).ToString(), "Сообщение",  MessageBoxButtons.OK,  MessageBoxIcon.Information,  MessageBoxDefaultButton.Button1,  MessageBoxOptions.DefaultDesktopOnly);
                       // Parent.Parent.Focus();
                        ShowWindow(Parent.Parent.Handle, 3); //выносим окно на передний план
                        SetForegroundWindow(Parent.Parent.Handle); //делаем окно активным
                        return;
                    }
                }
                catch (Exception) {
                    SystemSounds.Asterisk.Play();
                    MessageBox.Show("ВВЕИТЕ КОРРЕКТНЫЕ ДАННЫЕ по осям "+(i-1).ToString(), "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, MessageBoxOptions.DefaultDesktopOnly);
                    ShowWindow(Parent.Parent.Handle, 3); //выносим окно на передний план
                    SetForegroundWindow(Parent.Parent.Handle); //делаем окно активным
                    return;
